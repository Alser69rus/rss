...
async def geturl(url: str) -> str:
    async with aiohttp.ClientSession() as session:
        async with session.get(url, proxy='http://Evdokimow:evd@pkbgate:8080') as http:
            text = await http.text()
            return text


async def parse_xml(text: str):
    global count
    ...
    for item in items:
        ...
        if dic.get(guid) == newdic.get(guid):
            ...
            print('', f'{count:.1%}{guid} {title}')
            print(f'{count:.1%} \t {guid} \t {title}', file=f)
            f.close()


async def get_info(url: str):
    text = await geturl(url)
    await parse_xml(text)


with open('last.txt', 'rb') as f:
    dic = dict(pickle.load(f))

future = [get_info(url) for url in urls.values()]
